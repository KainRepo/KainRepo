--[[
		...
		Version History:
			Version: 1.14:
				Added PROdiction.
				Fixed Killsteal and added more combos.
				Added full Reborn compatibility.
...
function Vars()
	...
	QSpeed, WSpeed = 1.0, 3.0
	QDelay, WDelay = 234, 280
	QWidth, WWidth = 70, 105

	WRadius = 175

	if IsSACReborn and not FPSProblem_UseVIPPred then
		SkillQ = AutoCarry.Skills:NewSkill(false, _Q, QRange, "Dark Binding", AutoCarry.SPELL_LINEAR_COL, 0, false, false, QSpeed, QDelay, QWidth, true)
		SkillQCol = {spellKey = _Q, range = QRange, speed = QSpeed, delay = QDelay, width = QWidth, minions = true}		
		-- SkillW = AutoCarry.Skills:NewSkill(false, _W, WRange, "Tormented Soil", AutoCarry.SPELL_LINEAR_COL, 0, false, false, WSpeed, WDelay, WWidth, false)
	else
...
function CastQ(enemy)
	...
	if QReady and IsValid(enemy, QRange) and AutoCarry.PluginMenu.ComboQ then
		local col = nil
		if IsSACReborn then
			col = AutoCarry.GetCollision(SkillQCol, myHero, enemy)
		else
			col = AutoCarry.GetCollision(SkillQ, myHero, enemy)
		end
...
function CastW(enemy)
	if (not Target and not enemy) or not WReady then return false end
	if not enemy then enemy = Target end

	if enemy.canMove and EnemyCount(myHero, (WRange + WRadius)) < AutoCarry.PluginMenu.WEnemyCount then return false end

	local spellPos = GetAoESpellPosition(WRadius, enemy, SkillW.delay)

	if spellPos and (not enemy.canMove or AutoCarry.PluginMenu.ComboWGroup) and GetDistance(spellPos) <= WRange then
		if not enemy.canMove or EnemyCount(spellPos, WRadius) >= AutoCarry.PluginMenu.WEnemyCount then
			if debugMode then PrintChat("W AoE") end
			CastSpell(SkillW.spellKey, spellPos.x, spellPos.z)
			return true
		end
	elseif not spellPos and not enemy.canMove then
	...
	if not myHero.dead then
		for i, enemy in ipairs(GetEnemyHeroes()) do
			...
			if enemy and not enemy.dead then
				...
				if AutoCarry.PluginMenu.KillstealQ and enemyDistance < QRange and enemy.health <= (QDmg) then
					CastQ(enemy)
				elseif AutoCarry.PluginMenu.KillstealW and enemyDistance < WRange and enemy.health <= (WDmg) then
					CastSpell(SkillW.spellKey, enemy)
				elseif AutoCarry.PluginMenu.KillstealQ and AutoCarry.PluginMenu.KillstealW and enemyDistance < QRange and enemyDistance < WRange and enemy.health <= (QDmg + WDmg) then
					if CastQ(enemy) then
						CastSpell(SkillW.spellKey, enemy)
					end
				end
