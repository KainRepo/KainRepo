--[[
        ...
        Auto Carry Plugin - Nocturne PROdiction Edition
		Author: Kain
		...
		How to install:
			...
			Name the script EXACTLY "SidasAutoCarryPlugin - Nocturne.lua" without the quotes.
			Place the plugin in BoL/Scripts/Common folder.

		Features:

		Download: https://bitbucket.org/KainBoL/bol/raw/master/Common/SidasAutoCarryPlugin%20-%20Nocturne.lua

		Version History:
			Version: 1.1:
				Added visual option when can Ultimate.
			Version: 1.0:
				Release
...
function Vars()
	...
	-- RRange = {2000,2750,3500}

	if IsSACReborn then
		SkillQ = AutoCarry.Skills:NewSkill(false, _Q, QRange, "Duskbringer!", AutoCarry.SPELL_LINEAR, 0, false, false, QSpeed, QDelay, QWidth, false)
	else
	...
	if VIP_USER then
		PrintChat("<font color='#CCCCCC'> >> Kain's Nocturne - PROdiction 2.0 <</font>")
	else
		PrintChat("<font color='#CCCCCC'> >> Kain's Nocturne - Free Prediction <</font>")
	end
...
function Menu()
	...
	AutoCarry.PluginMenu:addParam("ComboQ", "Use Duskbringer", SCRIPT_PARAM_ONOFF, true)
	AutoCarry.PluginMenu:addParam("ComboW", "Use Shroud of Darkness", SCRIPT_PARAM_ONOFF, true)
	AutoCarry.PluginMenu:addParam("ComboE", "Use Unspeakable Horror", SCRIPT_PARAM_ONOFF, true)
	AutoCarry.PluginMenu:addParam("ComboR", "Use Paranoia", SCRIPT_PARAM_ONKEYDOWN, false, string.byte("T"))
	...
	ExtraConfig:addParam("DrawTargetInUltimateRange", "Draw Targets in Ultimate Range", SCRIPT_PARAM_ONOFF, true)
	...
	ExtraConfig:addParam("DrawQ", "Draw Duskbringer", SCRIPT_PARAM_ONOFF, true)
	ExtraConfig:addParam("DrawE", "Draw Unspeakable Horror", SCRIPT_PARAM_ONOFF, true)
	ExtraConfig:addParam("DrawR", "Draw Paranoia", SCRIPT_PARAM_ONOFF, true)
end
...
function PluginOnLoad()
	...
	if IsSACReborn then
		AutoCarry.Crosshair:SetSkillCrosshairRange(3600)
	else
		AutoCarry.SkillsCrosshair.range = 3600
	end
...
function Combo()
	if Target then
		...
		if AutoCarry.PluginMenu.ComboE and EReady and GetDistance(Target) <= ERange then
			CastSpell(_E, Target)
			if AutoCarry.PluginMenu.ComboW and WReady then
				CastSpell(_W, Target)
			end
		end
	end
...
function DrawRanges()
	if not ExtraConfig.DisableDrawCircles and not myHero.dead then
		...
		if ExtraConfig.DrawR and RReady and ((ExtraConfig.DrawFurthest and farSpell and farSpell == GetRRange()) or not ExtraConfig.DrawFurthest) then
			DrawCircle(myHero.x, myHero.y, myHero.z, GetRRange(), 0xFF0000) -- Red
		end
...
function FindFurthestReadySpell()
	...
	if ExtraConfig.DrawR and RReady and (not farSpell or GetRRange() > farSpell) then farSpell = GetRRange() end
...
--UPDATEURL=
--HASH=EA29FBDEA926C513B388CEDFE8BC8C67
