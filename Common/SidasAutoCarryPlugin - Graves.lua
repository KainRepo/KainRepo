...
function Vars()
	...
	if IsSACReborn then
		SkillQ = AutoCarry.Skills:NewSkill(false, _Q, QRange, "Buckshot", AutoCarry.SPELL_LINEAR, 0, false, false, QSpeed, QDelay, QWidth, false)
		SkillW = AutoCarry.Skills:NewSkill(false, _W, WRange, "Smoke Screen", AutoCarry.SPELL_LINEAR, 0, false, false, WSpeed, WDelay, WWidth, false)
		SkillE = AutoCarry.Skills:NewSkill(false, _E, ERange, "Quickdraw", AutoCarry.SPELL_LINEAR, 0, false, false, ESpeed, EDelay, EWidth, false)
		SkillR = AutoCarry.Skills:NewSkill(false, _R, RRange, "Collateral Damage", AutoCarry.SPELL_LINEAR, 0, false, false, RSpeed, RDelay, RWidth, false)
	else
	...
	SkillRPred = { spellKey = _R, range = RRange, speed = RSpeed, delay = RDelay, width = RWidth, configName = "collateralDamage"}
	...
	if VIP_USER then
		if FileExist(SCRIPT_PATH..'Common/Prodiction.lua') then
			PrintChat("<font color='#CCCCCC'> >> Kain's Graves - PROdiction 2.0 <</font>")
		end
	else
		PrintChat("<font color='#CCCCCC'> >> Kain's Graves - Free Prediction <</font>")
	end
end
...
function CastR(enemy)
	...
	if enemy and RReady and GetDistance(enemy) < RRange then
		...
		elseif AutoCarry.PluginMenu.MinREnemies > 1 and VIP_USER then
			predPos = AutoCarry.GetPrediction(SkillRPred, enemy)
			if predPos then
				local hitEnemyCount = 0
				local distance = myHero + (Vector(predPos) - myHero):normalized() * RRange
				local col = Collision(RRange, RSpeed * 1000, RDelay / 1000, RWidth)
