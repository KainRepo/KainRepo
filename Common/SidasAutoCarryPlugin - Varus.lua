--[[
		...
		Version History:
			Version: 2.3:
				Added PROdiction.
				Made changes to hold Q for more damage as long as possible if not in dangerous situation.
			Version: 2.2:
...
function Vars()
	...
	QRange, ERange, RRange = 1475, 925, 1075
	QSpeed, ESpeed, RSpeed = 1.85, 1.5, 1.95
	QDelay, EDelay, RDelay = 100, 242, 500
	QWidth, EWidth, RWidth = 60, 275, 80
	...
	if IsSACReborn then
		...
		SkillR = AutoCarry.Skills:NewSkill(false, _R, RRange, "Chain of Corruption", AutoCarry.SPELL_LINEAR_COL, 0, false, false, RSpeed, RDelay, RWidth, true)
	else
		...
		SkillR = {spellKey = _R, range = RRange, speed = RSpeed, delay = RDelay , width = RWidth, minions = true}
	end
	...
	if VIP_USER then
		if FileExist(SCRIPT_PATH..'Common/Prodiction.lua') then
			...
			qp = SetupPROdiction(_Q, QRange, QSpeed, QDelay, QWidth, myHero,
				function(unit, pos, castSpell)
					if QReady and GetDistance(unit) < QRange then
						FireQ(unit, pos, castSpell, false)
					end
				end)

			PrintChat("<font color='#CCCCCC'> >> Kain's Varus - PROdiction 2.0 <</font>")
		end
	else
		PrintChat("<font color='#CCCCCC'> >> Kain's Varus - Free Prediction <</font>")
	end
end
...
function Menu()
	...
	AutoCarry.PluginMenu:addParam("MaxQHarassDistance", "Max. Q Harass Range", SCRIPT_PARAM_SLICE, QRange, 0, QRange, 0) -- W stacks to Q
...
function DrawRanges()
	if not ExtraConfig.DisableDrawCircles and not myHero.dead then
		...
		if ExtraConfig.DrawR and RReady and ((ExtraConfig.DrawFurthest and farSpell and farSpell == RRange) or not ExtraConfig.DrawFurthest) then
...
function FindFurthestReadySpell()
	...
	if ExtraConfig.DrawR and RReady and (not farSpell or RRange > farSpell) then farSpell = RRange end
...
function PluginOnCreateObj(object)
	if object and object.valid and Target then
		if object.name == "VarusW_counter_02.troy" and GetDistance(object) <= QRange then
			for i, enemy in pairs(AutoCarry.EnemyTable) do
		...
		if object.name == "VarusW_counter_03.troy" and GetDistance(object) <= QRange then
			for i, enemy in pairs(AutoCarry.EnemyTable) do
				if ValidTarget(enemy) and TargetHaveBuff("varuswdebuff", enemy) then
...
function OldOnSendPacket(packet)
    ...
    if p:get("spellId") == _E and not (AutoCarry.MainMenu.AutoCarry or AutoCarry.MainMenu.LaneClear or AutoCarry.MainMenu.MixedMode or AutoCarry.PluginMenu.SlowE) then
    	p:block()
    end
end
--]]
...
function Harass()
	...
	if ValidTarget(target) then
		...
		if not IsMyManaLow() then
			if AutoCarry.PluginMenu.HarassE and EReady and targetDistance <= ERange then CastE(target) end
			if AutoCarry.PluginMenu.HarassQ and QReady and targetDistance <= QRange and targetDistance > TrueRange
				and targetDistance <= AutoCarry.PluginMenu.MaxQHarassDistance and ShouldReleaseQ(target, nil) then
				CastQ(target, false)
			end
		end
	end
...
function LaneClear()
	...
	for _, minion in pairs(AutoCarry.EnemyMinions().objects) do
		if ValidTarget(minion, ERange) and getDmg("E", minion, myHero) >= minion.health then AutoCarry.CastSkillshot(SkillE, minion) end
	end
	...
	for _, minion in pairs(AutoCarry.EnemyMinions().objects) do
		if ValidTarget(minion, ERange) then AutoCarry.CastSkillshot(SkillE, minion) end
	end
...
function LastHitE()
	...
	if killableMinions >= AutoCarry.PluginMenu.LastHitMinimumMinions then
		for _, minion in pairs(Minions) do
			if ValidTarget(minion, ERange) and EReady then AutoCarry.CastSkillshot(SkillE, minion) end
			return
		end
	end
	return
...
function CastEQAuto()
	...
	if QReady and (AutoCarry.PluginMenu.PrioritizeQ or not EReady) then
		...
		and ValidTarget(mostStacksEnemy, QRange) then
	...
	elseif EReady and ((not AutoCarry.PluginMenu.PrioritizeQ) or (AutoCarry.PluginMenu.PrioritizeQ and not QReady)) then
		...
		if mostStacksEnemy and not mostStacksEnemy.dead and (ProcStacks[mostStacksEnemy.networkID] == 3
			...
			and ValidTarget(mostStacksEnemy, ERange) and GetTickCount() > QTick + (GetQDelay(mostStacksEnemy)) then
...
function CastE(enemy)
	...
	if EReady and enemy and not enemy.dead and ValidTarget(enemy, ERange) then
		if EnemyCount(enemy, EWidth) > 1 then
			local spellPos = GetAoESpellPosition((EWidth / 2), enemy, EDelay)
...
function CastManualQ(mouse) -- cast Q to lowhp enemy or mousepos
	if mouse then
		...
		if lowHealthEnemy and ValidTarget(lowHealthEnemy, QRange) and getDmg("Q", lowHealthEnemy, myHero) >= lowHealthEnemy.health then
			...
			if mostStacksEnemy and ProcStacks[mostStacksEnemy.networkID] >= AutoCarry.PluginMenu.CarryMinWForQ and ValidTarget(mostStacksEnemy, QRange) then
				CastQ(mostStacksEnemy, false)
				return true
			else
...
function ShouldReleaseQ(enemy, pred)
	if not pred then pred = enemy end

	local QDelay = GetQDelay(pred)
	local isMinimumTime = (GetTickCount() - QTick >= QDelay)

	if not isMinimumTime then return false end

	local closestEnemy = FindClosestEnemy()
	local closestEnemyDistance = GetDistance(closestEnemy)
	local isEnemyRetreating = IsEnemyRetreating(enemy, pred)
	local enemyCount = EnemyCount(myHero, QRange)

	if QDelay >= 2000 then return true end

	if isEnemyRetreating and IsNearRangeLimit(pred, QRange) then
		return true
	end

	if enemyCount >= 1 and closestEnemyDistance < 400 then
		return true
	end

	if enemyCount <= 2 and closestEnemyDistance > 800 then
		return false
	end
...
function CastQ(Unit, mouse)
	...
	if Unit then
		...
		predic = qp:EnableTarget(Unit, true)
	elseif mouse then
		return FireQ(Unit, nil, _Q, mouse)
	end

	return false
end

function FireQ(unit, QPred, castSpell, mouse)
	if (QPred or mouse) and not Cast and ((mouse == 1 and GetTickCount() - QTick > ExtraConfig.WWaitDelay) or (QPred and ShouldReleaseQ(unit, QPred))) then
		if mouse then
	...
	if (QPred or mouse) and Cast and ((mouse == 2 and GetTickCount() - QTick > ExtraConfig.WWaitDelay) or (QPred and ShouldReleaseQ(unit, QPred))) then
		PQ2 = CLoLPacket(0xE6)
...
function CastR(enemy)
	...
	if RReady and enemy and not enemy.dead and ValidTarget(enemy, RRange) then
		if IsSACReborn then
			SkillR:Cast(enemy)
			return true
		else
...
function JungleClear()
	...
	if Target and ValidTarget(Target) then
		...
		if myHero:GetDistance(Target) <= ERange and EReady then
			if IsSACReborn then
				SkillE:Cast(Target)
			else
...
function Killsteal()
	...
	for _, enemy in pairs(AutoCarry.EnemyTable) do
		...
		elseif ValidTarget(enemy, TrueRange) and getDmg("AD", enemy, myHero) >= enemy.health then
			...
			return true
		elseif AutoCarry.PluginMenu.KillstealE and ValidTarget(enemy, ERange) and getDmg("E", enemy, myHero) >= enemy.health then
			CastE(enemy)
			return true
		elseif AutoCarry.PluginMenu.KillstealQ and ValidTarget(enemy, QRange) and getDmg("Q", enemy, myHero) >= enemy.health then
			CastQ(enemy, false)
			return true
 		elseif AutoCarry.PluginMenu.KillstealR and RReady and ValidTarget(enemy, RRange) and getDmg("R", enemy, myHero) >= enemy.health then
			CastR(enemy)
			return true
		end
	end
...
function SlowClosestEnemy()
	...
	if RReady and AutoCarry.PluginMenu.SlowR and EnemyCount(closestEnemy, RRange) >= 3 and ValidTarget(closestEnemy, RRange) then
		CastR(closestEnemy)
		return true
	end
