...
if IsSACReborn then
	SkillQ = AutoCarry.Skills:NewSkill(true, _Q, QMaxRange, "Bouncing Bomb", AutoCarry.SPELL_LINEAR_COL, 0, false, false, QSpeed, QDelay, QWidth, true)
	SkillW = AutoCarry.Skills:NewSkill(true, _W, WRange, "Satchel Charge", AutoCarry.SPELL_CIRCLE, 0, false, false, WSpeed, WDelay, WWidth, false)
	SkillE = AutoCarry.Skills:NewSkill(true, _E, ERange, "Hexplosive Minefield", AutoCarry.SPELL_CIRCLE, 0, false, false, ESpeed, EDelay, EWidth, false)
	SkillR = AutoCarry.Skills:NewSkill(true, _R, RRange, "Mega Inferno Bomb", AutoCarry.SPELL_CIRCLE, 0, false, false, RSpeed, RDelay, RWidth, false)
else
...
if VIP_USER then
	if FileExist(SCRIPT_PATH..'Common/Prodiction.lua') then
		...
		PrintChat("<font color='#CCCCCC'> >> Kain's Ziggs - PROdiction 2.0 <</font>")
	end
else
...
function Menu()
	...
	ZiggsExtraConfig:addParam("DoubleIgnite", "Don't Double Ignite", SCRIPT_PARAM_ONOFF, true)
...
function Ultimate()
	if Target ~= nil and AutoCarry.PluginMenu.Ultimate and (doUlt or ((Target.health + 30) < getDmg("R", Target, myHero) and IsValidHitChanceCustomR(Target))) then
		CastR()
	end
end
...
function CastQ()
	if Target ~= nil and not Target.dead then
		...
		elseif QReady and ValidTarget(Target, QMaxRange) then
			...
			if IsSACReborn then
				predic = tpQ:EnableTarget(Target, true)
			else
				predic = AutoCarry.GetPrediction(SkillQ, Target)
				FireQ(Target, predic, myHero:GetSpellData(_Q))
			end
		end
	end
end
...
function CastSkillshotBounce(skill, PredictedPos)
	if PredictedPos and AutoCarry.IsValidHitChance(SkillQ, Target) then
		...
		if CastPos and GetDistance(PredictedPos) < QMaxRange and IsQWallCheckOK(CastPos) and IsQWallCheckOK(EnemyPos) then
...
function CastR()
	if Target ~= nil and ValidTarget(Target, RRange) and not Target.dead then
		...
		if RReady and ValidTarget(Target, RRange * .98) then
			if IsSACReborn then
...
function PluginOnDraw()
	...
	if not ZiggsExtraConfig.DisableDraw and not myHero.dead then
		...
		if ZiggsExtraConfig.DrawQ and QReady and ((ZiggsExtraConfig.DrawFurthest and farSpell and farSpell == QRange) or not ZiggsExtraConfig.DrawFurthest) then
			DrawCircle(myHero.x, myHero.y, myHero.z, QMaxRange, 0x0099CC) -- Blue
		end

		if ZiggsExtraConfig.DrawW and WReady and ((ZiggsExtraConfig.DrawFurthest and farSpell and farSpell == WRange) or not ZiggsExtraConfig.DrawFurthest) then
			DrawCircle(myHero.x, myHero.y, myHero.z, WRange, 0xFFFF00) -- Yellow
		end
		
		if ZiggsExtraConfig.DrawE and EReady and ((ZiggsExtraConfig.DrawFurthest and farSpell and farSpell == ERange) or not ZiggsExtraConfig.DrawFurthest) then
...
function FindFurthestReadySpell()
	...
	if ZiggsExtraConfig.DrawW and WReady then farSpell = WRange end
	if ZiggsExtraConfig.DrawE and EReady and (not farSpell or ERange > farSpell) then farSpell = ERange end
	if ZiggsExtraConfig.DrawQ and QReady and (not farSpell or QMaxRange > farSpell) then farSpell = QRange end
		...
		for _, enemy in ipairs(GetEnemyHeroes()) do
			if ValidTarget(enemy) then
				if enemy ~= nil and enemy.team ~= myHero.team and not enemy.dead and enemy.visible and GetDistance(enemy) < IgniteRange and enemy.health < getDmg("IGNITE", enemy, myHero) then
					if ZiggsExtraConfig.DoubleIgnite and not TargetHaveBuff("SummonerDot", enemy) then
						CastSpell(ignite, enemy)
					elseif not ZiggsExtraConfig.DoubleIgnite then
						CastSpell(ignite, enemy)
					end
				end
			end
